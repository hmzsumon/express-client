...
import { useCreatePasswordMutation } from '@/features/auth/authApi';
...
const SetPassword = () => {
	...
	// validate password by length and @ least 1 number and 1 letter
	const validatePassword = (password: string) => {
		const re = /^(?=.*[A-Za-z])(?=.*[@])(?=.*\d)[A-Za-z@0-9]{6,}$/;
	...
	useEffect(() => {
		...
		if (isSuccess) {
			...
			if (user?.is_registration_done === false) {
				router.push({
	...
	return (
		<>
			...
			<div className='px-6 mx-auto my-10 md:w-6/12 '>
				<h1 className='text-2xl font-bold gradient-text '>Set Password</h1>
				<div className='my-8'>
